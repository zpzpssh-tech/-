...
# 회수 대상 공개정보 병합
_rp=os.path.join(SCR,"recover_public.json")
if os.path.exists(_rp):
    try:
        pub=json.load(open(_rp,encoding="utf-8"))
        for r in rows:
            if not r.get("카테고리","").startswith("회수 대상"): continue
            for k,v in pub.items():
                if k and (k in r["업체명"] or r["업체명"] in k or k.replace("(주)","") in r["업체명"]):
                    for f in ("홈페이지","대표전화","이메일","소재지"):
                        if v.get(f) and not r.get(f): r[f]=v[f]
                    if v.get("업종설명"): r["주요취급품목"]=r["주요취급품목"]+" / 공개정보: "+v["업종설명"]
                    if v.get("출처URL"): r["출처URL"]=r["출처URL"]+" ; "+v["출처URL"]
    except Exception as e: print("recover_public skip",e)
...
prio = {"상":0,"중":1,"하":2}
rows.sort(key=lambda r:(0 if r.get("카테고리","").startswith("회수 대상") else 1, prio.get(r.get("협업가능성","중"),1), r.get("카테고리",""), r.get("업체명","")))
...
guide = [
 ...
 ["회수 대상", "리자위와 결별한 前 파트너(함석민)가 응대하던 거래처. 리자위 쪽 생산 실적이 있으므로 최우선 컨택. 리스트 맨 위에 배치."],
 [""],
 ["주의사항"],
...
lines = [
 ["포지션 한 줄", "품목에 상관없이, 한국에서 필요한 제품과 사양만 주시면 중국 현지 제조공장 발굴 → 샘플 → 가격협상 → 생산 → 검수 → 통관·국내 납품까지 한 번에 연결하는 회사. (보호대는 '우리가 실제로 중국에서 만들어 팔고 있다'는 증거로만 언급)"],
 ["전화 첫 멘트", "안녕하세요, 올투게더나우 대표 ○○○입니다. 저희는 중국 현지 제조 파트너와 함께 굿즈·판촉물·패키지·생활용품·봉제·전자소품 등 품목 가리지 않고 공장 발굴부터 샘플, 생산, 검수, 국내 입고까지 한 번에 진행하는 회사입니다. 저희 자체 제품도 같은 방식으로 중국에서 만들어 네이버·쿠팡에서 팔고 있어서 품질·납기 관리는 직접 겪어본 경험이 있습니다. 귀사에서 중국 생산이 들어가는 건이 있으면 견적 비교부터 도와드리고 싶은데, 구매 담당 부서로 연결 가능할까요?"],
 ["핵심 차별점", "1) 단순 구매대행이 아니라 중국 현지 제조 파트너가 직접 공장을 찾고 생산을 관리. 2) 품목 제한 없음: 인쇄물·잡화·봉제·사출·금속·패키지·전자소품까지 공장 네트워크로 대응. 3) 저희도 자체 제품을 중국에서 만들어 파는 회사라 사고 유형(불량, 납기, 인증)을 알고 미리 막음. 4) 수출입 사업자로 세금계산서 발행, 통관, KC 인증 안내까지 국내에서 처리."],
 ["이메일 제목", "[제안] 중국 OEM 생산 원스톱 대행 (공장 발굴~검수~국내 납품, 품목 무관) - 올투게더나우"],
 ["이메일 본문", "안녕하세요, 올투게더나우 대표 ○○○입니다.\n\n저희는 중국 현지 제조 파트너와 함께, 품목에 상관없이 중국 생산이 필요한 제품을 아래 과정으로 원스톱 진행해 드립니다.\n\n1. 제품·사양·수량·예산만 알려주시면 중국 현지 파트너가 제조공장을 발굴\n2. 샘플 제작 및 국내 발송\n3. 가격 협상 및 견적서 제출\n4. 생산 진행 및 출하 전 검수 (사진·영상 보고)\n5. 통관 후 국내 지정 장소 납품, 세금계산서 발행\n\n대응 품목: 판촉물·굿즈(인쇄, 아크릴, 금속, 봉제, 사출), 패키지(박스·쇼핑백·파우치), 생활용품·주방용품, 스포츠·헬스용품, 전자소품(보조배터리·케이블 등) 외 다수.\n저희 자체 브랜드 제품도 같은 방식으로 중국에서 생산해 네이버·쿠팡에서 판매 중이라, 품질·납기 관리는 직접 겪어본 노하우로 진행합니다.\n\n진행 중인 건 하나로 견적 비교부터 해보셔도 좋습니다. 연락 주시면 바로 찾아뵙고 설명드리겠습니다.\n\n올투게더나우 대표 ○○○ / 전화 000-0000-0000 / 이메일 ○○○@○○○"],
 ...
 ["예상 질문 4", "Q. 무슨 품목까지 되나요? → A. 저희가 하는 건 특정 제품이 아니라 '공장을 찾아 생산을 관리하는 일'이라 품목 제한이 없습니다. 지금 진행 중인 품목 말씀해 주시면 그 공장부터 찾아드립니다."],
 ["회수 대상 멘트", "안녕하세요, 올투게더나우 ○○○ 대표입니다. 그동안 함석민 님을 통해 진행하셨던 중국 생산 건(예: 클립펜·온습도계, 캐릭터 굿즈, 패키지 박스)은 실제로는 저희 중국 파트너 리자위 공장에서 만들어졌습니다. 현재 함석민 님과는 거래가 정리되어, 앞으로는 저희가 한국 창구를 맡아 같은 공장·같은 품질로 직접 진행해 드립니다. 중간 단계가 하나 빠지는 만큼 단가와 소통 속도 모두 유리합니다. 진행 중이거나 예정된 건 있으시면 바로 견적 드리겠습니다."],
 ["회수 대상 주의", "1) 前 파트너 험담은 하지 말고 '거래가 정리됐다'까지만. 2) 상대가 함석민과 계속 거래 중일 수 있으니 첫 통화는 탐색 위주로. 3) 과거 생산 사진·샘플을 리자위에게 미리 받아두면 '같은 공장' 증명이 바로 됩니다. 4) 개인 휴대폰보다 회사 번호·메일로 먼저 연락."],
 ["후속 조치", "통화 후 24시간 안에 이메일로 회사소개(중국 공장 사진, 생산 사례 사진, 자사 제품 판매 페이지) + 견적 요청 양식(품목/사양/수량/희망단가/납기) 발송. 1주 뒤 재연락."],
]
